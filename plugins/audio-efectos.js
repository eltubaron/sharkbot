...
if (err) return m.reply(`*ACONTECEU ALGUM ERRO.. TENTE USAR OUTRO AUDIO QUE SEJA mp3. SE O PROBLEMA CONTINUAR REPORTE ESSE COMANDO  ${usedPrefix + command}*`)
let buff = await readFileSync(filename)
conn.sendFile(m.chat, buff, ran, null, m, true, {
type: 'audioMessage', 
ptt: true 
})})
} else return m.reply(`*RESPONDA UMA NOTA DE ÁUDIO OU VOZ PARA APLICAR O FILTRO USANDO ${usedPrefix + command}*`)
} catch (e) {
