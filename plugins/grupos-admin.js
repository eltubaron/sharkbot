...
const isCommand11 = /^(tagall|marcar|invocar|invocacion|todos|invocación)$/i.test(command)
...
await conn.reply(NumeroUser + '@s.whatsapp.net', `✨ *¡Oláa! @${NumeroUser}, @${user} ¡¡Te convidou!!*`, null, {mentions: [NumeroUser + '@s.whatsapp.net', m.sender]})    
await conn.sendMessage(NumeroUser + '@s.whatsapp.net', { text: `*Sou ${packname}, um Bot para WhatsApp.*\n\n:¨·.·¨:\n\`·. ${groupMetadata.subject}`, contextInfo: { externalAdReply: { title: '❤️ ¡Esperamos você no grupo!', body: '9999999999', thumbnailUrl: pp, sourceUrl: link, mediaType: 1, showAdAttribution: false, renderLargerThumbnail: true }}})
...
handler.command = /^(promote|daradmin|marcar|darpoder|demote|quitarpoder|quitaradmin|setwelcome|bienvenida|edit(?:ar)?wel(?:come)?|setbye|despedida|edit(?:ar)?(bye)?|setdesk|setdesc|newdesc|descripción|descripcion|cambiardesc|setname|newnombre|nuevonombre|cambiarnombre|cambiarpp|setpp(group|grup|gc)?|nuevolink|nuevoenlace|kick|echar|hechar|sacar|ban|group|grupo|tagall|invocar|invocacion|todos|invocación|prohibir|prohibit|privar|deprive|add|agregar|invitar|invite|añadir)$/i
handler.group = true
